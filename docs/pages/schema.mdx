...
the `to` option. This way the edge name doesn't have to match the table name
...
You have to set the `field` option to determine which side of the edge stores
the edge.
...
You have to set the `field` option to signify that the singular end of the edge
