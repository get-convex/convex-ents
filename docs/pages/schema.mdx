...
- Besides exporting the `schema`, which is used by Convex for schema validation,
  we also export `entDefinitions`, which include the runtime information needed
  to enable retrieving Ents via edges and other features.

## Fields

{/* TODO: Add support for unique fields, like "email" in Prisma's example */}
...
symmetrical edge). We can also provide the edge names specifically, from either
or both ends:
...
"user" respectively for each direction of the edge (since this is not a
symmetrical edge). We can also provide the edge names explicitly, from either or
both ends:
