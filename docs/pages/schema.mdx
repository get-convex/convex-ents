...
user. This is because the `edge` method was used on the "messages" ent. To
declare a many:many edge, you could replace it with the `edges` method.
...
If you wanted to have another field edge from profiles to users, you would have
to specify the field name. You can choose to specify the field name even if you
have only one edge:
...
You have to provide the field name on both ends of the edge (as `ref` and
`field` respectively).
...
symmetrical edge). You can also provide the edge names specifically, from either
or both ends, and pass the table name in as the `to` option:
...
ent. And since each user can have multiple associated messages, the foreign key
must be stored in the "messages" table. Just like for 1:1 edges, the field name
is inferred to be "userId" based on the "users" table name. You have to pass
`ref: true` to the `edges` method, to distinguish the edge from a many:many
edge.

If you wanted to have another field edge from messages to users, you would have
to specify the field name. You can choose to specify the field name even if you
have only one edge:
...
Similarly to field (1:many) edges, if you want to have multiple edges connecting
the same pair of tables, you have to provide the name of the table storing the
edge. You can do this even if you have only one edge:
...
As with 1:many edges, you can give the edges names:
