...
<Tabs items={["Example query", "mutation", "action", "schema"]}>
...
```ts filename="convex/ai.ts"
export const generate = internalAction({
  args: { id: v.id("prompts") },
  handler: async (ctx, { id }) => {
    const { text } = await ctx.table("prompts").getX(id);
    const response = await fetchResponse(text);
    await ctx.table("prompts").getX(id).patch({ response });
  },
});
```
