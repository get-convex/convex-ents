...
- 1:1 edge between ent A and ent B, ent A stores the edge.
  ...
  - When ent B is deleted, the ent A connected to it is deleted as well (which
    ...
    - Unless the edge from A to B is marked as
      [optional](/schema#optional-11-edges), in that case the ID field is unset
      and the deletion does not propagate.
- 1:many edge between ent A and ent B, ent A stores the edge.
  - When ent A is deleted, only _it_ is deleted.
  - When ent B is deleted, ents A connected to it are deleted as well (which
    might cause more edge and ent deletions).
    - Unless the edge from A to B is marked as
      [optional](/schema#optional-1many-edges), in that case the ID field is
      unset and the deletion does not propagate.
...
### Deleting optional 1:1 and 1:many edges

You can override the default behavior for optional 1:1 and 1:many edges, forcing
them to propagate deletion.

```ts
defineEntSchema({
  users: defineEnt({
    name: v.string(),
  }).edge("profile", { optional: true, deletion: "hard" }),
  profiles: defineEnt({
    bio: v.string(),
...
In this example, when a user is deleted, their profile is deleted as well, even
though a profile does not require the user edge.
