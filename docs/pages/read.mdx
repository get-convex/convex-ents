...
```ts
const usersWithMessages = await ctx.table("users").map((user) => {
  return {
    name: user.name,
...
```ts
const usersWithProfileAndMessages = await ctx.table("users").map((user) => {
...
const usersWithMessagesAndTags = await ctx.table("users").map(async (user) => ({
  ...user,
  messages: await user.edge("messages").map((message) => ({
    text: message.text,
...
export const list = query({
  ...
  handler: async (ctx) => {
    const usersWithMessagesAndProfile = await ctx
      ...
      .map((user) => ({
        ...user,
        profile: user.edgeX("profile").doc(),
      }));
  },
});
```
