...
data to any table. Therefore in vanilla Convex you must either use the
[new safer APIs](https://docs.convex.dev/database/reading-data/) which take
table names as their first argument or correctly use both
...
### Reading a single ent via an indexed field
...
const tasks = await Promise.all(
  [taskId1, taskId2].map((id) => ctx.db.get("tasks", id)),
);
```

</Aside>

### Reading multiple ents via an indexed field

```ts
const users = await ctx
  .table("users")
  .getMany("email", ["steve@apple.com", "bill@microsoft.com"]);
```
...
const users = await Promise.all(
  ["steve@apple.com", "bill@microsoft.com"].map((email) =>
    ctx.db
      .query("users")
...
Any index with a single field is supported. `getMany` does not support indexes
...
### Reading multiple ents via an indexed field or throwing

Retrieve a list of ents or throw an `Error` if any of the field values didn't
map to an existing ent:

```ts
const users = await ctx
  .table("users")
  .getManyX("email", ["steve@apple.com", "bill@microsoft.com"]);
```

Any index with a single field is supported. `getManyX` does not support indexes
over multiple fields.
...
const tags = await Promise.all(
  (
    await ctx.db
      ...
      .collect()
  ).map((edge) => ctx.db.get("tags", edge.tagsId)),
);
```
...
### Getting many:many edge IDs

If you're performing some kind of a "join" on the client, you might want the
server to only return the IDs of the ents related via a many:many edge, without
having to load all the related ents. You can do this with the `ids` method:
...
const tagIds = await Promise.all(
  (
    await ctx.db
      ...
      .withIndex("messagesId", (q) => q.eq("messagesId", messageId))
      .collect()
  ).map((edge) => edge.tagsId),
);
```

</Aside>

You can further chain the methods `paginate`, `take`, `first`, `firstX`,
`unique`, `uniqueX` to limit the number of IDs retrieved:

```ts
const firstTagId = await ctx
  .table("messages")
  .getX(messageId)
  .edge("tags")
  .ids()
...
const firstTagId = edge.tagsId;
```

</Aside>

To change the order in which the IDs are listed, use the `order` method before
the `ids` method:

```ts
const lastTagId = await ctx
  .table("messages")
  .getX(messageId)
  .edge("tags")
  .order("desc")
...
You can use the `map` method to transform the ID to another result:

```ts
const sharedIds = await ctx
  .table("messages")
  .getX(messageId)
  .edge("tags")
  .ids()
  .map(async (id) => [
    id,
    await ctx.table("messages").getX(anotherMessageId).edge("tags").has(id),
  ]);
```
...
const profile = await user.edgeX("profile");
```
