...
export const query = customQuery(
  ...
  customCtx(async (ctx) => {
    return {
      table: entsTableFactory(ctx.db, entDefinitions),
      db: undefined,
    };
  })
);
...
example `"oldTable"` and `"anotherOldTable"`). But remember that this is purely
a TypeScript enforcement, not a runtime one.
